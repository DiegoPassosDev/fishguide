...
```mermaid
graph TD
    APIsOficiais["APIs Oficiais"] --> FGEngine["FG Score Engine"]
    FGEngine --> DEngine["Decision Engine"]
    DEngine --> Memoria["Memória Coletiva da Pesca"]
    Memoria --> FishGuideAI["FishGuide AI"]
    FishGuideAI --> Usuário
```
